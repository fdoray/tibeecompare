...
//     2      ********   ******
    ...
    Segment segment;
    segment.set_thread(1);
    segment.set_startTs(0);
    segment.set_endTs(21);

    Segments segments;
    std::unordered_set<thread_t> excludedThreads;
    GetSegments(segment, builder, excludedThreads, &segments);
    ...
    std::set<Segment> expectedSegmentsSet = {b, c};
...
//     4   **************
    ...
    std::set<Segment> expectedSegmentsSet = {b, c, d};
    EXPECT_EQ(expectedSegmentsSet, segmentsSet);
}
