    ...
    Vertex** end)
{
    ...
    *start = startUP.get();
    ...
    // Add final vertex.
    auto& prevThread = (*verticesPerThread)[execution.startThread()];
    if (prevThread.empty())
    {
        base::tberror() << "No previous vertex on thread "
                        << execution.startThread() << "." << base::tbendl();
        return false;
    }
    auto* prevVertex = prevThread.back().get();
    Vertex::UP endUP(new Vertex(execution.startThread(), execution.endTs()));
    *end = endUP.get();
    prevVertex->hout = endUP.get();
    endUP->hin = prevVertex;
    (*verticesPerThread)[execution.startThread()].push_back(std::move(endUP));
...
void AssignLevels(Vertex* start, Vertex* end)
{
    ...
    while (!queue.empty())
    {
        ...
        if (point->level == kInvalidLevel)
        {
            ...
            {
                if (point->hout->vin == nullptr ||
                    point->hout->vin->level != kInvalidLevel)
                {
    ...
    Vertex** end)
{
    if (!GenerateExecutionGraphInternal(
            execution, links, verticesPerThread,
            start, end))
    {
        return false;
    }
